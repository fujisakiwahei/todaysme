...
<template>
  <main class="login">
    ...
    <div v-else class="login__card">
      ...
      <!--
        TODO: 新規登録 / パスワード再設定 (未実装)

        MVP 現在は招待制（開発者本人のみ）のためサインアップ動線は持たない。
        将来オープン化する際の設計メモ:

        - 表示位置: フォーム下に footer ブロック（design-tone/pages/login.html 準拠）
            「初回の方は <a>アカウントを作成</a> · <a>パスワード再設定</a>」
        - ルート:
            /signup           … メール + パスワード新規登録（Supabase signUp + email confirm）
            /password-reset   … reset link 送信フォーム
        - スタイル: .login__footer (fs 12 / color text-muted / link は color-info で underline)
        - Google ログインのみで運用する場合はサインアップは不要。
          パスワード再設定リンクのみを残す案も検討すること。
      -->
    </div>
  </main>
</template>
