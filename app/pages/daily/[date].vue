...
const prevDate = computed(() => shiftDate(dateParam.value, -1));
const nextDate = computed(() => shiftDate(dateParam.value, 1));

const dateLabel = computed(() => {
  const [y, m, d] = dateParam.value.split("-").map(Number);
  const dt = new Date(Date.UTC(y!, m! - 1, d!));
  const weekday = ["日", "月", "火", "水", "木", "金", "土"][dt.getUTCDay()];
  return `${dateParam.value}（${weekday}）`;
});

const isToday = computed(() => {
  if (!summary.value) return false;
  return isTodayInTimezone(summary.value.target_date, summary.value.timezone);
});

const timezone = computed(() => summary.value?.timezone ?? "Asia/Tokyo");
...
const lastSyncedAt = computed<string | null>(() => {
  ...
  const ms = summary.value.sync_statuses
...
const meAggregate = computed(() => {
  if (!summary.value?.wake_range) return null;
  const rs = new Date(summary.value.wake_range.start).getTime();
  const re = new Date(summary.value.wake_range.end).getTime();
  ...
  let activeMs = 0;
  for (const ev of summary.value.timeline.calendar) {
    activeMs += overlapMs(ev.start_at, ev.end_at);
  }
  for (const t of summary.value.timeline.toggl) {
...
const timelineSpan = computed(() => {
  if (!summary.value?.wake_range) return null;
  const start = new Date(summary.value.wake_range.start).getTime();
  const end = new Date(summary.value.wake_range.end).getTime();
...
const preWakeSleep = computed<SleepTimelineEntry | null>(() => {
  if (!summary.value?.wake_range) return null;
  const rangeStart = new Date(summary.value.wake_range.start).getTime();
  return (
    summary.value.timeline.sleep.find(
...
const inRangeSleep = computed<SleepTimelineEntry[]>(() => {
  ...
  return summary.value.timeline.sleep.filter((s) => s.id !== pre?.id);
});

const calendarEvents = computed<CalendarTimelineEntry[]>(
  () => summary.value?.timeline.calendar ?? [],
);
const togglEntries = computed<TogglTimelineEntry[]>(
  () => summary.value?.timeline.toggl ?? [],
);
    ...
    <div class="daily__container">
      <header class="daily__topbar">
        ...
        <div class="daily__refresh">
          ...
          <span class="daily__refresh-time">
