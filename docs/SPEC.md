...
- **覚醒時間**: その日最初の起床から現在（過去日は wake range 終端）までの時間から、同範囲内の二度寝・仮眠区間を差し引く。
- **アクティブ時間割合**: Calendar + Toggl の時間から睡眠区間との重複を除外して合算し、覚醒時間で割る。Calendar と Toggl 同士の重複は表示用近似値として合算する。
- **未記録時間**: 覚醒時間からアクティブ時間を差し引く（下限 0）。
